...
//    @WithMockUser(username = "a", roles = {"b"}, password = "c")
    ...
    public void shouldReturnUsers() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/users").accept(MediaType.APPLICATION_JSON))
                ...
                .andExpect(jsonPath("$.[0].username").value("krevetka")) // JSONPath expression language is like XPath for JSON and can be used to traverse JSON structures and extract data from them. It begins with $, which represents the root of the JSON document.
